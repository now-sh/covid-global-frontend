    ...
    <div v-if="!loading">
      ...
      <div v-else>
        ...
        <div class="row border border-primary" v-if="data && data.length">
          <div class="card text-center col-4 border border-danger" v-for="historical in this.data" :key="historical.province">
              <h5 class="card-title mt-2 text-danger">  {{ historical.province }} {{ historical.country }} </h5>
              <p class="card-text mt-2 bt-3">
                <span v-for="(value, key) in historical.timeline.cases" :key="key" class="text-warning">
